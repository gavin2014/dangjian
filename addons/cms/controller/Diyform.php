...
class Diyform extends Base
{
    ...
    public function index()
    {
        ...
        $is_get = Db::name($diyform['table'])->where('archives_id',$archives_id)->where('user_id', $this->auth->id)->find();
        if ($is_get){
            return $this->redirect('index/cms.diyform/mypost');
        }
        ...
        $this->view->assign('archives_id',$archives_id);
        $archives_name =Db::name('cms_archives')->where('id',$archives_id)->field('title')->find()['title'];
        $this->view->assign('archives_name',$archives_name);
    ...
    public function post()
    {
        if ($this->request->isPost()) {
          ...
          $is_get = Db::name($diyform['table'])->where('archives_id',$row['archives_id'])->where('user_id', $row['user_id'])->find();
            if (!$is_get){
                try {
...
//                return $this->redirect('index/cms.diyform/mypost');
                $this->error('您此活动已经提交过,如需修改请前往个人中心修改哟','index/cms.diyform/mypost');
            }
